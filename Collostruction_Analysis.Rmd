...
* Research focus
  ...
  * Interest centers on the use of the English *Perfect* aspect across different presidents.
  * The goal involves comparing how this grammatical structure appears across speakers.

* Methodological idea

  * Perfect constructions can be extracted from sentences through a regular expression.
  * This approach supports systematic pattern identification in corpus data.

* Working assumption

  * For this exercise, Perfect constructions include all sequences that match the pattern `have/has/had + ...-en/ed`.
  * This assumption remains simple, practical, transparent for exploratory analysis.
...
- This is our current data in a data frame format (document-based):
...
For actual research, POS information can be very useful for retrieving constructions more accurately. 

You may therefore want to utilize other existing POS taggers for more sophisticated preprocessing (e.g., [spacyr](https://cran.r-project.org/web/packages/spacyr/vignettes/using_spacyr.html)).
...
- Joint frequency = number of times `AUX(have/had/has) + given` occurs.
...
A sample result file is available in the root directory `2026-01-XX XX-XX-XXXX.csv`.
...
1. **Purpose of the tutorials**: This series aims to show the power of R in corpus processing, quantitative analysis. Its popularity continues to grow within corpus research.

2. **Expectations about code**: You need not understand every line of code. Focus on overall logic, workflow, practical application.

3. **Strengths of R**: R supports statistical analysis, text preprocessing, data visualization within one environment. This versatility makes it suitable for many corpus tasks.

4. **Value of script templates**: The templates allow easy extension to more complex tasks through targeted modification of key parts.

5. **Role of AI support**: AI assistance makes corpus analysis with R easier, reduces coding effort, lowers entry barriers.

6. **Suggested learning approach**: Practice with templates, apply them to personal data, refine scripts through iterative experimentation.
