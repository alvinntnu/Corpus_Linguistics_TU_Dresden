...
::: info
As described by Hadley Wickham [@hadley2017], tidy data has a specific structure:
...
```{r engpipe, purl= F, echo=F, fig.cap = "Computational Text Processing Flowchart"}
...
To handle Chinese data, however, we need to be more careful. We probably need to define own ways of tokenization method in `unnest_tokens(..., token = ...)`. 

We will discuss the principles for Chinese text processing in a later [chapter](#chinese-text-processing).
```
...
:::{.try}
Could you explain the difference between the total numbers of unigrams and bigrams?
:::
...
Dispersion is an important construct and many more sophisticated quantitative metrics have been proposed to more properly operationalize this concept. Please see @gries2021a.
...
```{exercise}
Get yourself familiar with another R package for creating word clouds, `wordcloud2`, and re-create a word cloud as requested in Exercise \@ref(exr:wordcloud1) but in a fancier format, i.e., a star-shaped one. (Criteria: Frequency >= 20; Max Number of Words Plotted = 400)
```
...
Cooccurrence frequency data for a word pair, *w~1~* and *w~2~*, are often organized in a contingency table extracted from a corpus, as shown in Figure \@ref(fig:collo1).
...
```{r echo=F}
corp_us_collocations_president %>%
  filter(str_detect(bigram, "domestic concerns")) ->temp
temp
```
For example, the above table shows that while in "`r temp$Year_President[1]`", there was only one token of the bigram *`r temp$bigram[1]`*, the bigram occurred **`r temp$O11[1]`** times in the entire corpus.
...
str_view_all(corp_us_tidy$text[3] %>% str_sub(1,8000),"ha(d|ve|s) (\\w+ ){1,3}\\w+(en|ed)")
```
